...
// For delays
#define DELAY_S 1000000	// second
#define DELAY_MS 1000	// millisecond
#define DELAY_US 1	// microsecond

void setupLCD();	// full setup
void test_default();	// test lcd
...
void writeGraphicToLCD(char * byte, unsigned char transform) {
	...
	if (transform & ROTATE) {
		...
		while (c != 0) {
			...
			( * byte & 0x80) ? (block[0] |= c) : (block[0] &= ~c); * byte++;
			c <<= 1;
		}
	} else {
		while (c < 8) {
